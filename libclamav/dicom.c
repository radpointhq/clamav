...
/* Dataset integers follow the transfer-syntax byte order. The File Meta group
 * is always Explicit VR Little Endian and uses cli_readint* directly; only the
 * dataset walkers go through these. (VR chars and value bytes are never
 * byte-swapped — only tag group/element and lengths.) */
static uint16_t dicom_rd16(const dicom_metadata *meta, const uint8_t *p)
{
    if (meta->big_endian) {
        return (uint16_t)(((uint16_t)p[0] << 8) | p[1]);
    }
    return (uint16_t)cli_readint16(p);
}

static uint32_t dicom_rd32(const dicom_metadata *meta, const uint8_t *p)
{
    if (meta->big_endian) {
        return ((uint32_t)p[0] << 24) | ((uint32_t)p[1] << 16) |
               ((uint32_t)p[2] << 8) | (uint32_t)p[3];
    }
    return (uint32_t)cli_readint32(p);
}
                                             ...
                                             size_t *endpos)
{
    ...
    /* Encapsulation only occurs with JPEG-family transfer syntaxes, which are
     * always Explicit VR Little Endian, so item headers are read LE here. */
    while (off + 8 <= end) {
        const uint8_t *p = fmap_need_off_once(map, off, 8);
        uint16_t group, element;
        uint32_t length;
                                      ...
                                      size_t *endpos)
{
    ...
    while (off + 8 <= lim) {
        ...
        group   = dicom_rd16(meta, p);
        element = dicom_rd16(meta, p + 2);
        length  = dicom_rd32(meta, p + 4);
        off += 8;
    ...
    while (off + 8 <= end) {
        ...
        if (meta->explicit_vr && TAG_ITEM != tag) {
            ...
            if (vr_has_long_length(vr)) {
                ...
                length     = dicom_rd32(meta, q);
                header_len = 12;
            } else {
                length     = dicom_rd16(meta, p + 6);
                header_len = 8;
            }
            is_sq = ('S' == vr[0] && 'Q' == vr[1]);
        } else {
        ...
        /* Implicit VR has no VR field, so a defined-length SQ is
         * indistinguishable from a binary blob by tag alone (no dictionary).
         * Heuristic: if the value begins with an item tag (FFFE,E000), treat
         * it as a sequence and recurse. A false positive only costs a parse
         * attempt that bails to CL_EPARSE (the value is still raw-scanned by
         * the engine), so this is safe. PixelData is handled separately. */
        if (!meta->explicit_vr && !is_sq && length >= 8 && TAG_PIXEL_DATA != tag) {
            const uint8_t *peek = fmap_need_off_once(map, off + header_len, 4);
            if (NULL != peek) {
                uint16_t pg = dicom_rd16(meta, peek);
                uint16_t pe = dicom_rd16(meta, peek + 2);
                if (TAG_ITEM == DICOM_TAG(pg, pe)) {
                    cli_dbgmsg("dicom: implicit-VR defined-length SQ inferred at %zu\n", off);
                    is_sq = true;
                }
            }
        }
...
/*
 * CVE-2019-11687 (PE/DICOM "PEDICOM") and ELFDICOM: the 128-byte preamble is
 * left unspecified by the DICOM standard and the "DICM" magic is only checked
 * at offset 128, so a file can be simultaneously a valid DICOM image and a
 * valid executable by hiding an exe header in the preamble. A conformant DICOM
 * preamble is all zeros.
 *
 * This MUST run independently of file typing: a real polyglot has its exe magic
 * at offset 0, so the engine types it as CL_TYPE_MSEXE/ELF and never dispatches
 * to cli_scandicom. It is therefore called from the generic scan path for every
 * file, and gated on "DICM" actually being present at offset 128 so it only
 * fires for genuine DICOM polyglots, not for ordinary executables.
 */
cl_error_t cli_dicom_check_preamble_polyglot(cli_ctx *ctx)
{
    const uint8_t *p;
    const char *heur = NULL;

    if (NULL == ctx || NULL == ctx->fmap || !SCAN_HEURISTICS) {
        return CL_SUCCESS;
    }
    /* Require the DICOM magic at offset 128 — this is the structural anomaly
     * (an executable header in front of a DICOM file), not a bare executable. */
    p = fmap_need_off_once(ctx->fmap, DICOM_PREAMBLE_SIZE, 4);
    if (NULL == p || 0 != memcmp(p, "DICM", 4)) {
        return CL_SUCCESS;
    }
    p = fmap_need_off_once(ctx->fmap, 0, 4);
    if (NULL == p) {
        return CL_SUCCESS;
    }

    if (0x4D == p[0] && 0x5A == p[1]) {
        heur = "Heuristics.DICOM.ExecutableInPreamble.PE"; /* MZ */
    } else if (0x7F == p[0] && 'E' == p[1] && 'L' == p[2] && 'F' == p[3]) {
        heur = "Heuristics.DICOM.ExecutableInPreamble.ELF";
    } else if ((0xFE == p[0] && 0xED == p[1] && 0xFA == p[2] && (0xCE == p[3] || 0xCF == p[3])) ||
               ((0xCE == p[0] || 0xCF == p[0]) && 0xFA == p[1] && 0xED == p[2] && 0xFE == p[3]) ||
               (0xCA == p[0] && 0xFE == p[1] && 0xBA == p[2] && 0xBE == p[3])) {
        heur = "Heuristics.DICOM.ExecutableInPreamble.MachO"; /* also Java class CAFEBABE */
    } else if ('#' == p[0] && '!' == p[1]) {
        heur = "Heuristics.DICOM.ExecutableInPreamble.Script";
    }

    if (NULL != heur) {
        cli_dbgmsg("dicom: foreign executable header in preamble (%s)\n", heur);
        return cli_append_potentially_unwanted(ctx, heur);
    }
    return CL_SUCCESS;
}
...
cl_error_t cli_scandicom(cli_ctx *ctx)
{
    ...
    /* Note: the preamble polyglot heuristic (CVE-2019-11687 / ELFDICOM) runs in
     * the generic scan path (cli_dicom_check_preamble_polyglot), not here, so it
     * fires even when the engine types a polyglot as CL_TYPE_MSEXE/ELF. */
