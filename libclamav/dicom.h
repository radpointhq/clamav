...
/*
 * Flag a DICOM preamble polyglot (CVE-2019-11687 / ELFDICOM): an executable
 * header at offset 0 in front of the DICOM "DICM" magic at offset 128. Runs
 * from the generic scan path for every file (gated on "DICM" at offset 128),
 * so it fires even when the engine types the polyglot as an executable rather
 * than as DICOM. Returns CL_VIRUS when it raises a (heuristic) alert.
 */
